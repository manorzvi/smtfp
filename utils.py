...
class PackedFP32:
    ...
    def from_fp32(self, x: Union[float, float32, np.float32]) -> Tuple[str, str, str]:
        assert isinstance(x, float) or isinstance(x, float32) or isinstance(x, np.float32)
    ...
    def floor_by_mantissa(self, x: Union[float, float32, np.float32]) -> np.float32:
        sign, exponent, _ = self.from_fp32(x)
        mantissa = '00000000000000000000000'
        x = self.to_fp32(sign, exponent, mantissa)
        return x
    
    def ceil_by_mantissa(self, x: Union[float, float32, np.float32]) -> np.float32:
        sign, exponent, _ = self.from_fp32(x)
        mantissa = '00000000000000000000000'
    ...
    def zero_mantissa_to_closest_exponent(self, x: Union[float, float32, np.float32]) -> np.float32:
        ...
        if mantissa[0] == '0':
            x = self.floor_by_mantissa(x)
        else:
            x = self.ceil_by_mantissa(x)
        return x
...
def test():
    ...
    for x in X:
        ...
        # x3 = PackedFP32().floor_by_mantissa(x)
        # x3 = PackedFP32().ceil_by_mantissa(x)
